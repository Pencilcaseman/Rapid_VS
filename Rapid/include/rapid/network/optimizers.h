...
namespace rapid
{
	namespace network
	{
		namespace optim
		{
			...
			{
				int initialized = 0;
				ndarray::Array<t> defaultValue;
				ndarray::Array<t> value;

				inline void setValue(const ndarray::Array<t> &val)
				{
					initialized = 1;
					value.set(val.copy());
				}

				inline ndarray::Array<t> getValue() const
				{
					return initialized ? value : defaultValue;
				}
			};
			...
			struct Config
			{
				...
				NDArray<t> velocity;             // ndarray::Array of same shape as w storing a moving average of gradients
			...
			/// <returns></returns>
			template<typename t>
			inline OptimOutput<t> sgd(ndarray::Array<t> &w, const ndarray::Array<t> &dw, Config<t> &config)
			{
			...
			/// <returns></returns>
			template<typename t>
			inline OptimOutput<t> sgdMomentum(ndarray::Array<t> &w, const ndarray::Array<t> &dw, Config<t> &config)
			{
				if (config.initialized == 0)
				{
					config.initialized = 1;
			...
			/// <returns></returns>
			template<typename t>
			inline OptimOutput<t> rmsprop(ndarray::Array<t> &x, const ndarray::Array<t> &dx, Config<t> &config)
			{
				if (config.initialized == 0)
				{
					config.initialized = 1;
			...
			inline OptimOutput<ty> adam(ndarray::Array<ty> &x, const ndarray::Array<ty> &dx, Config<ty> &config)
			{
				if (config.initialized == 0)
				{
					config.initialized = 1;
